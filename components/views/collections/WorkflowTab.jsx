  ...
  return (
    <div className="animate-fade-in">
      <div className="bg-white rounded-2xl p-5 mb-5 card-shadow border border-slate-100">
        ...
        <p className="text-[13px] text-heading">Track cases through each stage of the collections process — from initial notice to recovery action.</p>
      </div>
        ...
        {collectionsWorkflow.stages.map((stage, i) => (
          <div key={stage.name} className="flex items-center gap-2">
            ...
            {i < collectionsWorkflow.stages.length - 1 && (
              <svg className="w-5 h-5 text-subtle flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-[11px] text-heading font-mono">{c.id} &middot; {c.daysOverdue}d overdue</div>
                      </div>
                      <div className="text-[14px] font-bold font-mono text-body">{c.amount}</div>
                    </div>
