  ...
  return (
    <div className="animate-fade-in">
      <SectionCard className="mb-5">
        <h3 className="text-[15px] font-bold text-heading mb-1">AI Agent Configuration</h3>
        <p className="text-[13px] text-heading">Manage AI model versions, fraud detection sensitivity, and autonomous agent configurations.</p>
      </SectionCard>
      ...
      <div className="bg-white rounded-2xl card-shadow border border-slate-100 overflow-hidden overflow-x-auto mb-5">
        <div className="px-5 py-3 border-b border-slate-100">
          <h4 className="text-[13px] font-bold text-body">Global AI Settings</h4>
        </div>
          ...
          {systemConfig.ai.map((config) => (
            <div key={config.key} className="flex items-center justify-between px-5 py-4 hover:bg-slate-50/50 transition-colors">
              <div className="flex-1">
                <div className="text-[13px] font-semibold text-body">{config.label}</div>
                <div className="text-[11px] text-heading mt-0.5">{config.key}</div>
              </div>
              <div className="flex items-center gap-3">
                {config.type === "toggle" ? (
                  <div className={`w-10 h-5 rounded-full flex items-center px-0.5 transition-colors cursor-pointer ${config.value === "Enabled" ? "bg-success-400" : "bg-slate-300"}`}>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl py-2 px-3 text-[13px] text-body font-mono min-w-[200px] text-right">
                    {config.value}
                  </div>
                )}
      ...
      <SectionCard className="mb-4">
        <h4 className="text-[14px] font-bold text-heading mb-0.5">Agent Instances</h4>
        <p className="text-[11px] text-heading">Individual AI agent status, model versions, and behaviour toggles.</p>
      </SectionCard>
        ...
        {aiAgents.map((ag) => (
          <div key={ag.name} className="bg-white rounded-2xl p-5 card-shadow border border-slate-100">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h4 className="text-[14px] font-bold text-heading">{ag.name}</h4>
                <div className="text-[11px] text-heading mt-0.5">Model: <span className="font-mono text-body">{ag.model}</span></div>
              </div>
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="text-center">
                <div className="text-[18px] font-bold text-primary">{ag.accuracy}%</div>
                <div className="text-[10px] text-heading">Accuracy</div>
              </div>
              <div className="text-center">
                <div className="text-[11px] text-slate-500">{ag.lastTrained}</div>
                <div className="text-[10px] text-heading">Last Trained</div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                <div key={toggle.label} className="flex items-center justify-between">
                  <span className="text-[12px] text-body">{toggle.label}</span>
                  <div className={`w-9 h-[18px] rounded-full flex items-center px-0.5 transition-colors cursor-pointer ${toggle.enabled ? "bg-success-400" : "bg-slate-300"}`}>
      ...
      <SectionCard className="mb-4">
        <h4 className="text-[14px] font-bold text-heading mb-0.5">Agent Behaviour Thresholds</h4>
        <p className="text-[11px] text-heading">Performance and operational limits for each agent.</p>
      </SectionCard>
      ...
      <DataTable
        ...
        renderRow={(t, i) => (
          <tr key={i} className="border-b border-slate-50 whitespace-nowrap hover:bg-slate-50/50 transition-colors">
            <td className="px-5 py-3 text-[12px] font-semibold text-body">{t.agent}</td>
            <td className="px-5 py-3 text-[12px] text-slate-500">{t.metric}</td>
            <td className="px-5 py-3 text-[12px] font-mono text-body">{t.value}</td>
            <td className="px-5 py-3 text-[12px] font-mono text-heading">{t.limit}</td>
