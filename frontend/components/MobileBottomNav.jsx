...
const adminNavItems = [
  { id: "executive", label: "Home", icon: <HomeIcon /> },
  { id: "members", label: "Members", icon: <MembersIcon /> },
  { id: "loans", label: "Loans", icon: <LoansIcon /> },
  { id: "deposits", label: "Deposits", icon: <DepositsIcon /> },
  { id: "more", label: "More", icon: <MoreIcon /> },
];

// Member bottom nav items
const memberNavItems = [
  { id: "member_dashboard", label: "Home", icon: <HomeIcon /> },
  { id: "my_chitfunds", label: "Chit Funds", icon: <ChitFundsIcon /> },
  { id: "my_loans", label: "Loans", icon: <LoansIcon /> },
  { id: "my_deposits", label: "Deposits", icon: <DepositsIcon /> },
...
export default function MobileBottomNav({ onMorePress }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 md:hidden safe-area-bottom">
      <div className="flex items-center justify-around px-2 py-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`${isActive ? "text-success-500" : ""}`}>
                {item.icon}
              </div>
              <span className={`text-[10px] font-medium ${isActive ? "text-success dark:text-success-400" : ""}`}>
                {item.label}
              </span>
              {isActive && (
                <div className="absolute bottom-1 w-5 h-0.5 bg-success-500 rounded-full" />
              )}
            </button>
          );
        })}
