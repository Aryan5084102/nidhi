    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white rounded-2xl card-shadow border border-slate-100 overflow-hidden overflow-x-auto mb-6">
        <div className="px-5 py-4 border-b border-slate-100">
          <h3 className="text-[15px] font-bold text-heading">Latest Compliance Check Results</h3>
        </div>
          ...
          <tbody>
            {complianceChecks.map((row) => (
              <tr key={row.check} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                <td className="px-5 py-3 text-[13px] text-body font-medium">{row.check}</td>
                ...
                <td className="px-5 py-3"><span className={`font-mono text-xs font-bold ${row.score >= 90 ? "text-success" : row.score >= 60 ? "text-warning" : "text-danger-500"}`}>{row.score}</span></td>
                <td className="px-5 py-3 text-[11px] text-heading">{row.next}</td>
              </tr>
            ))}
          </tbody>
        </table>
            ...
            <div key={r.report} className="flex items-start gap-3 bg-slate-50 rounded-xl p-4 border border-slate-100">
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="text-[13px] font-semibold text-body">{r.report}</div>
                    <div className="text-[11px] text-heading">Filed: {r.filed} &middot; Type: {r.type}</div>
                  </div>
        ...
        <div className="flex flex-col gap-3">
          {exceptionLog.map((ex, i) => (
            <div key={i} className="flex items-start gap-3 bg-slate-50 rounded-xl p-4 border border-slate-100">
              <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${ex.resolution === "Resolved" ? "bg-success-500" : ex.resolution === "Mitigated" ? "bg-blue-500" : ex.resolution === "Pending" ? "bg-warning-500" : "bg-danger-500"}`} />
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <span className="text-[13px] font-semibold text-body">{ex.exception}</span>
                  <span className="text-[10px] text-heading">{ex.time}</span>
                </div>
