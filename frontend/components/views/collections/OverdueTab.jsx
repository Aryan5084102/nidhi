      ...
      <div className="bg-white rounded-2xl p-5 mb-5 card-shadow border border-slate-100">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-[13px] text-heading">Monitor all overdue loans with severity-based prioritisation for collection action.</p>
          </div>
          <div className="hidden sm:flex gap-2">
            <div className="bg-danger-50 rounded-xl px-3 py-2 text-center border border-danger-200/60">
              <div className="text-lg font-bold text-danger-500 font-mono">{overdueLoans.filter((l) => l.daysOverdue >= 90).length}</div>
            ...
            <div className="bg-warning-50 rounded-xl px-3 py-2 text-center border border-warning-200/60">
              <div className="text-lg font-bold text-warning font-mono">{overdueLoans.filter((l) => l.daysOverdue < 60).length}</div>
            ...
            <div key={loan.id} className={`bg-white rounded-2xl p-5 card-shadow border border-slate-100 hover:shadow-md transition-all duration-300`}>
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-[15px] font-bold text-heading">{loan.member}</span>
                    ...
                    <div className="text-[11px] text-heading font-mono mt-0.5 truncate">{loan.memberId} &middot; {loan.id} &middot; {loan.loanType}</div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-slate-50 rounded-xl p-3">
                  <div className="text-[10px] text-heading uppercase tracking-wider mb-1">Total Loan</div>
                  <div className="text-[13px] font-semibold text-body font-mono">{loan.totalLoan}</div>
                </div>
                <div className="bg-slate-50 rounded-xl p-3">
                  <div className="text-[10px] text-heading uppercase tracking-wider mb-1">EMI Amount</div>
                  <div className="text-[13px] font-semibold text-body font-mono">{loan.emiAmount}</div>
                </div>
                <div className="bg-slate-50 rounded-xl p-3">
                  <div className="text-[10px] text-heading uppercase tracking-wider mb-1">Last Payment</div>
                  <div className="text-[13px] font-semibold text-body">{loan.lastPayment}</div>
                </div>
                <div className="bg-slate-50 rounded-xl p-3">
                  <div className="text-[10px] text-heading uppercase tracking-wider mb-1">Collateral</div>
                  <div className="text-[13px] font-semibold text-body">{loan.collateral}</div>
                </div>
              </div>
            </div>
          );
