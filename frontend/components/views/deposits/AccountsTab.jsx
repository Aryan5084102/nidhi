...
export default function AccountsTab() {
  ...
  return (
    <div className="animate-fade-in">
        ...
        <div className="flex gap-2 overflow-x-auto pb-0.5" style={{ scrollbarWidth: "none" }}>
          {["All", "Fixed Deposit", "Recurring Deposit", "Savings Deposit"].map((f) => (
            <button key={f} onClick={() => setFilterType(f)}
              className={`rounded-xl px-4 py-2 text-xs cursor-pointer transition-all duration-150 border whitespace-nowrap shrink-0 ${filterType === f ? "bg-primary-50 border-primary-300 text-primary font-semibold" : "bg-white border-slate-200 text-heading hover:border-slate-300 hover:text-body"}`}>
          ...
          <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors whitespace-nowrap">
            ...
            <td className="px-5 py-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-[13px] font-semibold text-body">{a.memberName}</div>
                  <div className="text-[10px] text-heading">{a.memberId}</div>
                </div>
              </div>
            </td>
      ...
      />
      {filtered.length === 0 && (
        <div className="text-center py-10 text-[13px] text-heading">No deposit accounts found</div>
      )}
    </div>
  );
}
