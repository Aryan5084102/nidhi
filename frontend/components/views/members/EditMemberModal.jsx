...
export default function EditMemberModal({ member, onClose, onSave }) {
  ...
  const inputClass = "w-full bg-white border border-slate-200 rounded-xl py-2 px-3 text-sm text-body outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-500/10 transition-all";
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-fade-in">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
          <div>
            ...
            <input value={form.name} onChange={(e) => handleChange("name", e.target.value)} className={inputClass} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.risk} onChange={(e) => handleChange("risk", e.target.value)} className={`${inputClass} cursor-pointer`}>
            ...
            <div>
              ...
              <select value={form.kyc} onChange={(e) => handleChange("kyc", e.target.value)} className={`${inputClass} cursor-pointer`}>
          ...
          <div className="pt-3 border-t border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={form.nomineeName} onChange={(e) => handleChange("nomineeName", e.target.value)} placeholder="Full name of nominee" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Relation</label>
                <select value={form.nomineeRelation} onChange={(e) => handleChange("nomineeRelation", e.target.value)} className={`${inputClass} cursor-pointer`}>
              ...
              <div>
                ...
                <input value={form.nomineeAadhaar} onChange={(e) => handleChange("nomineeAadhaar", e.target.value)} placeholder="XXXX-XXXX-1234" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Nominee PAN</label>
                <input value={form.nomineePan} onChange={(e) => handleChange("nomineePan", e.target.value)} placeholder="ABCDE1234F" className={`${inputClass} uppercase`} />
              </div>
            </div>
          </div>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3">
          ...
          <button
            onClick={() => onSave(form)}
            className="bg-primary text-white rounded-xl px-5 py-2 text-xs font-semibold cursor-pointer hover:bg-primary-700 transition-colors"
          >
            Save Changes
          </button>
        </div>
